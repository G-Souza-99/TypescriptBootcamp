{
    ...
    function computeScore(word: string): number {
        const letters: (string | any)[] = word.toUpperCase().split('');
        return letters.reduce((accum, curr) => accum += getPointsFor(curr), 0);
    }

    function getPointsFor(letter: object): number {
        const lettersAndPoints: any = [
            ['AEOIULNRST', 1],
            ['DG', 2],
            ['BCMP', 3],
            ['FHVWY', 4],
        ...
        return lettersAndPoints.reduce((computedScore: any, pointsTuple: [any, any]) => {
            const [letters, score] = pointsTuple;
            if (letters.split('').find((ll: object) => ll === letter)) {
    ...
    const defaultGreeting: string = greet('hello');
    ...
    // function pushToCollection(item: string | boolean | never[], collection: object): Array<object[]> {
    ...
    // i couldnt understand how to make this work
}